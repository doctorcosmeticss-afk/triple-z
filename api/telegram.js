...
function formatOrderMessage(order) {
  ...
  message += `📅 <b>Date:</b> ${new Date(order.createdAt).toLocaleString('en-US', { 
    timeZone: 'Africa/Cairo', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit'
  })}\n\n`;
  ...
  if (order.notes && order.notes.includes('Alt phone:')) {
    ...
    if (altPhone) message += `📞 <b>Phone 2:</b> ${altPhone}\n`;
  }
  ...
  switch(order.paymentMethod) {
    case 'cash_on_delivery': message += 'Cash on Delivery'; break;
    case 'vodafone_cash': message += 'Vodafone Cash'; break;
    case 'instapay': message += 'InstaPay'; break;
    default: message += order.paymentMethod;
  }
  message += `\n`;

  if (order.paymentMethod !== 'cash_on_delivery') {
    if (order.payerName) message += `👤 <b>Payer:</b> ${order.payerName}\n`;
    if (order.payerAccount) message += `📱 <b>Account:</b> ${order.payerAccount}\n`;
    if (order.transferAmount) message += `💸 <b>Transferred:</b> ${order.transferAmount} EGP\n`;
  }
  ...
  if (order.discount > 0) message += `🎟️ <b>Discount:</b> -${order.discount} EGP\n`;
  if (order.promoCode) message += `🏷️ <b>Promo:</b> ${order.promoCode}\n`;
...
module.exports = { sendOrderNotification, sendTelegramMessage };